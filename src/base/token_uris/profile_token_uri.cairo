...
pub mod ProfileTokenUri {
    ...
    fn get_attributes(tokenid: u256, mint_timestamp: u64) -> Array<felt252> {
        let tokenid_felt: felt252 = tokenid.try_into().unwrap();
        let timestamp_felt: felt252 = mint_timestamp.try_into().unwrap();
        let tokenid_byte: ByteArray = tokenid_felt.try_into().unwrap();
        let tokenid_byte_len: felt252 = tokenid_byte.len().try_into().unwrap();
        ...
        attributes.append(tokenid_felt);
        attributes.append('"},{"trait_type":"HEX ');
        attributes.append('ID","value":"');
        attributes.append(tokenid_felt); // TODO to hex string 
        attributes.append('"},{"trait_type":"DIGITS"');
        attributes.append(',"value":"');
        attributes.append(tokenid_byte_len);
    ...
    fn get_json(tokenid: u256, mint_timestamp: u64) -> Array<felt252> {
        let tokenid_felt: felt252 = tokenid.try_into().unwrap();
        let timestamp_felt: felt252 = mint_timestamp.try_into().unwrap();
        let mut json = ArrayTrait::<felt252>::new();
        json.append('{"name":"Profile #');
        json.append(tokenid_felt);
